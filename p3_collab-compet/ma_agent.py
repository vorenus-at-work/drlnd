"""Wrapper class managing different agents in the environment.
    Help multiple agent to work together.
    For the project, the agents are ddpg agents.
    Just run the agents functions for multiple times
"""
import copy
import os
import random
...
# wrapper for ddpg-agent used for other 2 projects
from ddpg_agent import Agent, ReplayBuffer, device
from model import Actor, Critic

BUFFER_SIZE = int(10e6)         # replay buffer size
...
class ma_agent:
    ...
    def __init__(self, num_agents=2, state_size=24, action_size=2):
        ...
        self.agents = [Agent(state_size, action_size, i+1, random_seed=0) for i in range(num_agents)]
